...
const submit = () => {
    form.post(route('modules.management.store'), {
        preserveScroll: true,
        onSuccess: () => {
            form.reset();
...
<template>
    ...
    <AppLayout :breadcrumbs="breadcrumbs">
        <div v-if="userRole === 'admin' || userRole === 'superuser' && userStatus !== 'not active'" class="flex h-full flex-1 flex-col gap-6 rounded-xl p-4 sm:p-6 bg-gray-50 dark:bg-gray-900">
            <!-- Header Section -->
            <div>
                <h1 class="text-xl sm:text-2xl font-bold text-gray-800 dark:text-white">Create Module</h1>
                <p class="text-gray-600 dark:text-gray-300">Add a new loan module to the system</p>
            </div>
