...
import { stateSelection, citySelection, MalaysianState } from '@/lib/address';
...
const checkICNumberAvailability = debounce(async (icNum: string) => {
    ...
    try {
        ...
        const response = await axios.post(route('api.check-ic-number'), { ic_num: icNum });
...
<template>
    ...
    <AppLayout :breadcrumbs="breadcrumbs">
        <div v-if="userRole === 'admin' && userStatus !== 'not active'" class="flex h-full flex-1 flex-col gap-6 rounded-xl p-4 sm:p-6 bg-gray-50 dark:bg-gray-900">
            <!-- Header Section -->
            <div>
                <h1 class="text-xl sm:text-2xl font-bold text-gray-800 dark:text-white">Create New User</h1>
            ...
            <div class="space-y-6">
                <form @submit.prevent="submit" class="space-y-6">
                    ...
                    <!-- Form Actions -->
                    <div class="bg-white dark:bg-gray-800 rounded-xl shadow p-4 sm:p-6 flex flex-col sm:flex-row sm:justify-end gap-3 sm:gap-4">
